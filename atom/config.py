...
class QuantizationConfig():
    def __init__(self, config: PretrainedConfig = None):
        ...
        if self.hf_quant_config is None:
            self.global_quant_config = LayerQuantConfig(quant_type=QuantType.No, quant_dtype=self.torch_dtype)
            self.quant_method = None
            return

        self.quant_method = self.hf_quant_config.get("quant_method", None)
        if self.quant_method == "quark":
            layer_quant_config_dict = cast(dict[str, Any], self.hf_quant_config.get("layer_quant_config"))
            for layer_name, layer_cfg in layer_quant_config_dict.items():
                self.layer_quant_config[layer_name] = self.parse_quark_config_dict(layer_cfg)

            global_quant_config_dict = cast(dict[str, Any], self.hf_quant_config.get("global_quant_config"))
            self.global_quant_config = self.parse_quark_config_dict(global_quant_config_dict)
    ...
    def parse_other_config(self):
        ...
        self.global_quant_config = LayerQuantConfig(quant_type=quant_type, quant_dtype=quant_dtype, is_dynamic=is_dynamic)
    ...
    def parse_quark_config_dict(self, config: dict) -> LayerQuantConfig:
        ...
        return LayerQuantConfig(quant_type = quant_type,
    ...
    def get_layer_quant_config(self, layer_name: str) -> LayerQuantConfig:
        ...
        layer_quant_config = self.global_quant_config if layer_quant_config is None else layer_quant_config
    ...
    #     """Backward-compatible dict-style access, delegates to global_quant_config."""
    #     if key in ("quant_type", "quant_dtype", "is_dynamic"):
    #         return self.global_quant_config[key]
    #     if key == "quant_method":
    #         return self.quant_method
    #     if key == "exclude_layers":
    #         return self.exclude_layers or []
    #     raise KeyError(key)
